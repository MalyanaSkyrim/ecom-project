...
const healthReplySchema = z
  .object({
    status: z.string().meta({ description: 'Status of the server' }),
    uptime: z.string().meta({ description: 'Uptime of the server' }),
    docURL: z.url().meta({ description: 'URL of the API documentation' }),
    apiVersions: z.string().meta({ description: 'List of API versions' }),
  })
  .meta({ description: 'Reply for the health check' })
